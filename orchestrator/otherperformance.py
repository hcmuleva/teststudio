...
def deploy_jmeter_pods(total_pods, ns):
    ...
    with open(file_path, 'r') as f:
        ...
        deploypod(ns)
        time.sleep(10)
def  parallel_execution(namespace, primary_pod_name,confgid,userid,wll_data,auttype):
    ...
    run_test = f'kubectl exec -it -n {namespace} "{primary_pod_name}" -- /bin/bash /load_test app/sample.jmx -l app/answer.jtl -e -o app/results'
    ...
    if process.returncode == 0:
        ...
        #copy that result folder to local
        current_datetime=datetime.now()
        timestamp=current_datetime.strftime("%Y-%m-%d_%H%M")
        folder_name=f"Perf_{auttype}_{timestamp}"
        ...
        copy_result_file=f"kubectl cp {primary_pod_name}:/app/results {folder_path} -n {namespace}"
        ...
        payload ={"data":{"rawdata":stdout_result,"reporturl":reporturl, "config":confgid, "users_permissions_user": userid,"totalthreads":str(wll_data['threadcount']),"loops":str(wll_data['loop_count']), "total_workers":str(wll_data['total_workers']),"aut":str(wll_data['aut']), "duration":str(duration),"summary":str(summary), "status":str(test_status), "testtype":"PERFORMANCE"}}
...
def run_device_test(json_obj):
    ...
    try:
        ...
        controller_csv=f'{controller_url_env}{json_obj["data"]["csvfile"]}'
        response_csv=requests.get(controller_csv)
        ...
        print("Test Execution and Work load list==================")
        for workload_item in workload_list:
            ...
            temp_dir_csv = tempfile.mkdtemp()
            temp_file_csv = os.path.join(temp_dir_csv, "csv.zip")
            with open(temp_file_csv,"wb") as f:
                f.write(response_csv.content)
                dir_csv,filepath_csv=os.path.split(temp_dir_csv)
            ...
            if current_os == "Windows":
                ...
                for i in range(len(worker_pod_name)):
                    ...
                    copy_test_csv_file=f"kubectl cp {filepath_csv}\csv.zip -n {namespace} {worker_pod_name[i]}:/"
                    install_unzip_copy=f'kubectl exec -it {worker_pod_name[i]} -n {namespace} -- /bin/bash -c "apt-get update && apt-get install -y unzip && unzip -oj /csv.zip -d /app/csv"'
                    subprocess.run(copy_test_file_1, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True, text=True, check=True)
                    subprocess.run(copy_test_csv_file, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True, text=True, check=True)
                    subprocess.run(install_unzip_copy, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True, text=True, check=True)
        ...
        for key, value in primary_pod_name_object.items():
            ...
            thread = threading.Thread(target=parallel_execution, args=(key, value,confgid,userid,wll_data,auttype))
...
def runperformancetest(json_obj):
    result = run_device_test(json_obj)
    return result